--[[
    ...
    -- * Script Name: RainNailTrail.lua
    -- * Script Authors: RNT Squad
    -- * Script Version: 3.0

    -- * Discord: https://discord.gg/9FH2WrAjhy
...
function entity_c.get_condition()
    ...
    if (velocity > 5) then
        ...
        if (ui.get_keybind_state(keybinds.automatic_peek)) then
            return e_conditions.Peek
    end
        return e_conditions.Running
    end
    return e_conditions.Stand
end
...
ui_ragebot.double_tap_label = menu_manager_c.new("Ragebot", "Auto Stop"):label()
...
ui_anti_aim.preset = menu_manager_c.new("Anti-Aim", "AA Preset"):combo("None", "Aggressive", "Dynamic", "Smart Jitter", "Motion", "Fake Model", "Agressive v2", "Tank AA")

ui_anti_aim.roll_angles = menu_manager_c.new("Anti-Aim", "Roll Anti-Aim [Beta]"):checkbox()
...
ui_anti_aim.slowwalk_mod = menu_manager_c.new("Anti-Aim", "Enable Slowwalk Speed"):checkbox()
...
ui_anti_aim.fake_lag = menu_manager_c.new("Anti-Aim", "Enable FLBS"):checkbox()
ui_anti_aim.fake_lag_condition = function() return ui_anti_aim.fake_lag:get() end
ui_anti_aim.fake_lag_types = menu_manager_c.new("Anti-Aim", "Fake Lag Types", true, ui_anti_aim.fake_lag_condition):combo("Random", "Switch")
...
ui_visuals.crosshair_indicators = menu_manager_c.new("Visuals", "Crosshair Indicators"):combo("Disabled", "Standart","Full")
...
ui_visuals.panel = menu_manager_c.new("Visuals", "Panel"):checkbox()
ui_visuals.scope = menu_manager_c.new("Visuals", "Viewmodel in scope"):checkbox()
ui_visuals.sense_indicators = menu_manager_c.new("Visuals", "Gamesense Indicators"):checkbox()
...
ui_visuals.accent = menu_manager_c.new("Visuals", "Colour", true, ui_visuals.accent_condition):color_picker()
...
ui_global.seperator = menu_manager_c.new("Support Link",""):label()

ui_global.discord_link = menu_manager_c.new("Support Link", "Export to Console", "Export to console"):button(function()
    local link = ("9FH2WrAjhy"):gradient({158, 245, 255, 255}, {255, 255, 255, 255})
...
keybinds_c.list = {
    ...
    ["Hide Shots"] = keybinds.hide_shots,
...
function watermark_c.handle()
    ...
    local text = ("%s | %s "):format(script_name..".lua", nickname)
...
function userp()
    ...
    render.text(fonts.verdana.panel, screen.x/2 - 955 , screen.y/2 - 160, color(255,255,255,255),"#RainNainTrail.lua - version: v3 - "..uname,false,false)
...
cheat.notify("Recode author: Deyn1337#7768")
cheat.notify("Пожалуйста, отключите стандартные логи Рейвтрипа.")

cheat.push_callback("on_shot", function(shot_info) 
if shot_info.result == "Spread" then
    cheat.notify("[ RNT.lua ] Unregistered shot at " .. shot_info.target_name .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. " | Reason: spread") 
elseif shot_info.result == "Occlusion" then 
    cheat.notify("[ RNT.lua ] Unregistered shot at " .. shot_info.target_name .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. " | Reason: occlusion") 
elseif shot_info.result == "Hit" then   
    cheat.notify("[ RNT.lua ] Registered shot at " .. shot_info.target_name .. " | Hitbox: " .. shot_info.server_hitbox .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. "" )
elseif shot_info.result == "resolver" then  
    cheat.notify("[ RNT.lua ] Unregistered shot at " .. shot_info.target_name .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. " | Reason: bad resolver")
elseif shot_info.result == "unregistered" then  
    cheat.notify("[ RNT.lua ] Unregistered shot at " .. shot_info.target_name .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. " | Reason: death")
elseif shot_info.result == "invalid backtrack tick" then 
    cheat.notify("[ RNT.lua ] Unregistered shot at " .. shot_info.target_name .. " | Hitchance: "  .. shot_info.hitchance .. " | Damage: " .. shot_info.server_damage  .. " | Reason: bad prediction")
	end
end)
    ...
    if (event:get_name() == "player_death") then
        ...
        if attacker_to_player == lp_idx then
            phrases = {"go use neverlose ",
                ...
                "iq -1 ",
                "Weave moment ",
                "крякоюзер присоеденился к серверу.  ",
                "RNT.lua Забустила ",                
                "Сколько глобус не крути, все равно не мой хуй приземлишся)))))))))))))))  ",
                "жалко нищенку ",
                "Death outside ",
                "kill в консоль, plz ",
                "Обидно от рейвтрипа в еблище получить? ",
                "разбей клаву, clown ",
                "clown ",
                "Пляшешь, как мой хуй у тебя в глотке ",
                "запачкаал еи лицо валшебнай палачкай XP ",
                "xd ",
                "bruh ",
                "cringe ",
                "oh no, c r i n g e ",
                "stop use acidrech.tech, use RNT.lua ",
                "Ты знаешь что акулы убивают 5 человек за год? Похоже у тебя есть конкуренты. ",                
                "Выключи кс. Просто выйди на улицу и подойди к ближайшему дереву и извинись за то, что тратишь кислород ",
                "Один из тех, чья мать хотела шнурки завязать ",
                "Мамочка sperma.lua в Yaicekletka.cc заинжектила? ",
                "Ниже твоего кд только твой iq ",
                " *DEAD* Опущеный: бож чел ти бот слитий кидай дискорт еблан бездарь боже ",
                "Сколько можно падать, прикупи RNT.lua. Ах да, она бесплатная ",
                "Только не юзай RNT.lua, не позорь ",
                "Rain. ",
                "Nail. ",
                "Trail. ",
                "еле-еле 1 кд ",
                "накакаешь на следующей мапе ",
                " продай свой топ кфг пжпжппжпж ",
                "Твой плейстайл доказательство того, что мастурбация ухудшает зрение",
                "Посмотри Кислоту, может научишся чему ",
                "Посмотри Репачино, может научишся чему ",
                "Купите ему Неверлуз, мб с лоу таба выйдет ",
                "R N T . L U A ",
                "Не устану повотрять, RNT.lua > life ",
                "ds v chat, nisheta ",
                "obijaet script v igre?(((((( ",
                ") ",
                "2 ",
                "Твое место в доте. ",
                "Твое место у параши. ",
                "Твое место в майнкрафте. ",
                "Твое место в сапёре. ",
                "Твое место в дет доме. ",
                "350iq moment ",
                "Rawetrip - fly with us. ",
                "RNT.lua - fly with us. ",
                "Охотся за вантапами вместе с RNT.lua ",
                "RNT.lua - технологии будующего ",
                "RNT.lua - Начни побеждать в 1 клик. ",
                "только зайдя на RNT Squad, твоя мать останется не тронутой ",
                "RNT - Rain Nail Trail ",
                "Смысла от твоего чита столько же, как и от твоего можна ",
                "обидел треш толк в игре? Скачай RNT.lua ",
                " B E S T  F R E E  L U A ",
                "Крякоюзер покинул наше шоу по раздаче еблетов ",
                "Тебе даже скар не поможен ",
                "Кряк пандоры имба, согласен броууууу????? ",
                "Кряк Сосалити.луз не спасает против RNT.lua? ",
                "Кодер RNT.lua - Deyn1337#7768 ",
                "Хочешь луашечку на рейв, да с резольвером скита? RNT.lua - https:// discord.gg/9FH2WrAjh ",
                "Зайди - не позорься https://discord.gg/9FH2WrAjh ",
                "Юзер пандорки земли занюхнул ",
                "https:// discord.gg/9FH2WrAjhy ",
                "RNT.lua https:// discord.gg/9FH2WrAjhy ",
                "Ez4ka ",
                "Esp -> Leave -> Close CS:GO ",
                "Кто на сервер псину пустил? ",
                "disconnect в консоль ",
                "пиздалон хуебарный, чучело пиздогундливое",
                "Trash Talk by pussydestroyer69#5843 ",
                "сотри свое еблище наждачкой ",
                "засунь свою ладонь в мясорубку ",
                "твой интелект на уровне твоего бати, а его кстати посадили за изнасилование котенка ",
                "сын наркоманки, твоя мать подралась за дозу дезаморфина с тремя бомжами ",
                "залезь в будку нахуй, а то ща батя твой бухой припрется и начнет избивать твою мамашу ",
                "сын чумазого шахтера, перелопать органы своей бабушки ",
                "пиздодрачильник на хер вздетый, залезь в пизду своей матери обратно ",
                "козлопасина штопанная, какого это принимать хуи вместо пищи? ",
                "если бы за каждую твою смерть у тебя отнимали 1 хромосому, ты бы стал тупее сульфаниломида ",
                "пиздобузина защемленная, зачем ты вычищаешь остатки спермы из своего ануса во время игры ??? ",
                "слона заморского залупа отсосала у высокопочтеного Юзера RNT.lua ",
                "хуидло в бабьем сарафане, иди стену пробури своим ебасосником ",
                "ты настолько позорный, что я унижаю тебя даже когда ебу твою мать ",
                "схуежуённый дрочехвостик, сонную артерию себе перережь ",
                "пиздоебательный недоебок, я твоей мамаше черепуху скрошил ",
                "пиздюк блошиный хули сосешь так скачай RNT.lua и меньше позорься ",
                "сын лаваша ",
                "твой родной брат сын шлюхи ",
                "скройся туберкулезница ",
                "hs by RNT.lua ",
                "при каждом выстреле твою мать трахают ",
                "нацепи намордник ",
                "удаляйся с жизни ",
                "окунись в лужу спермы ",
                "позорище умерло ",
                "ливай ",
                "деда закосплеил??????????? ",
                "щеночек упал??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? ",
                "прикупил инвайт в сосалити, но забыл оплатить операцию на глаза? ",
                "Опять у хвх кинга каникулы ",
                "owned ",
                "O W N E D ",
                "owned by RNT.lua ",
                "RNT.lua юзер унизил hvhkinglegend1337 ",
                "Твое место в школе ",
                "Зря пикнул, пупсик ",
                "Пикнул - получил еблище ",
                "Пикнул с RNT.lua - дал еблище ",
                "Разрыв твоего ебала напоминает взрыв в Teardown ",
                "Интересный факт - игру Teardown назвали в честь того, что я использую тебя, как тир ",
                "Пулька в ебальцо залетела, как твоя сестра ",
                "Передерни на RNT.lua и продолжи играть ",
                "Один из тех, у кого правая рука накачена от дрочки, а левая от вытерания слезок с ебла ",
                "скароюзер, пук пук ",
                "R N T ",
                "Я влетел в тебя, как спермотазоид в твою мамку, только не выпал раньше ",
                "RNT.lua > Gamesense ",}
                random = math.random(1,169)

...
cheat.notify("Melancholia V2 Loaded, Have Fun!")
-- #region: Lua Cache
local flow_ui = {
    Info = {
        AboutLua = {},
...
local sub_tub = ui.add_combobox("Melancholia Functions", {""}) 

-- Info
flow_ui.Info.AboutLua.Welcome = ui.add_label("Welcome Back!")
flow_ui.Info.AboutLua.Name = ui.add_label("Script Name: Melancholia Free")
flow_ui.Info.AboutLua.Version = ui.add_label("Script Version: V2")
flow_ui.Info.AboutLua.Creator = ui.add_label("Made By: astraxzy#3574")

flow_ui.Info.AboutUser.Username = ui.add_label("Username: " .. engine.get_gamename())
flow_ui.Info.AboutUser.PCname = ui.add_label("PC Name: " .. globalvars.get_winuser())
flow_ui.Info.AboutUser.IQ = ui.add_label("IQ: " .. math.random(1,120))
-- End
...
-- AA 
-- Break Lag Comp
flow_ui.AA.BLC.Enabled = ui.add_checkbox("Enable BLC")
flow_ui.AA.BLC.Speed = ui.add_sliderint("BLC Update Speed", 1,10)
flow_ui.AA.BLC.InfoLabel = ui.add_label("Break Lag Compensation Work only without exploits!")
-- End

-- Presets
flow_ui.AA.Presets.AAPreset = ui.add_combobox("AA Preset", {"None", "Jitter"})
-- End 

-- Custom Jitter
flow_ui.AA.CustomJitter.Type = ui.add_combobox("Jitter Type", {"None", "Offset", "Center", "Random"})
flow_ui.AA.CustomJitter.Delta = ui.add_sliderint("Jitter Delta", -90,90) 
flow_ui.AA.CustomJitter.LeftLimit = ui.add_sliderint("Left Limit", -180,180) 
flow_ui.AA.CustomJitter.RightLimit = ui.add_sliderint("Right Limit", -180,180) 
-- End
-- End
...
-- Visuals
-- Indicators
flow_ui.Visual.Indicators.Enabled = ui.add_checkbox("Enable Indicators")
flow_ui.Visual.Indicators.anim_type = ui.add_combobox("Indicators Animation Type", {"Off", "Alpha Only", "Side", "Twinkle"})
flow_ui.Visual.Indicators.position_y = ui.add_sliderint("Indicators Y", 0,engine.get_screen_size().y)
flow_ui.Visual.Indicators.accent = ui.add_colorpicker("Indicators Accent")
flow_ui.Visual.Indicators.sub_accent = ui.add_colorpicker("Addictive Color")
flow_ui.Visual.Indicators.show_state = ui.add_checkbox("Show Player State")
flow_ui.Visual.Indicators.state_color = ui.add_colorpicker("Player State Color")
flow_ui.Visual.Indicators.animated_state = ui.add_checkbox("Animate State Changes")
flow_ui.Visual.Indicators.scope_change = ui.add_checkbox("Change Position In Scope")
flow_ui.Visual.Indicators.scope_change_amount = ui.add_sliderint("Amount", 1,200)
-- End
-- End
...
-- Misc
-- Legbreaker
flow_ui.Misc.LegBreaker.Enabled = ui.add_checkbox("Enable LegBreaker")
flow_ui.Misc.LegBreaker.Type1 = ui.add_combobox("First Type", {"Default", "Backward Slide", "Force Slide"})
flow_ui.Misc.LegBreaker.Type2 = ui.add_combobox("Second Type", {"Default", "Backward Slide", "Force Slide"})
flow_ui.Misc.LegBreaker.changespeed = ui.add_sliderint("Change Speed", 1,10)
-- End

-- Clantag
flow_ui.Misc.Clantag.Enabled = ui.add_checkbox("Enable Clantag")
-- End
-- End
--- #endregion
...
local function FlowtechUI()
    ...
    --- @Menu
    -- Info 
    for _, v in pairs(flow_ui.Info.AboutLua) do
        v:set_visible(tab:get() == 0 and sub_tub:get() == 0)
    end
    for _, v in pairs(flow_ui.Info.AboutUser) do
        v:set_visible(tab:get() == 0 and sub_tub:get() == 1)
    end
    -- end

    -- Indicators
    for _, v in pairs(flow_ui.Visual.Indicators) do
        v:set_visible(tab:get() == 3 and sub_tub:get() == 1 and flow_ui.Visual.Indicators.Enabled:get())
    end
    flow_ui.Visual.Indicators.scope_change_amount:set_visible(flow_ui.Visual.Indicators.scope_change:get() and tab:get() == 3 and sub_tub:get() == 1 and flow_ui.Visual.Indicators.Enabled:get())
    flow_ui.Visual.Indicators.sub_accent:set_visible(flow_ui.Visual.Indicators.anim_type:get() == 2 and tab:get() == 3 and sub_tub:get() == 1 and flow_ui.Visual.Indicators.Enabled:get())
    flow_ui.Visual.Indicators.animated_state:set_visible(flow_ui.Visual.Indicators.show_state:get() and tab:get() == 3 and sub_tub:get() == 1 and flow_ui.Visual.Indicators.Enabled:get())
    flow_ui.Visual.Indicators.state_color:set_visible(flow_ui.Visual.Indicators.show_state:get() and tab:get() == 3 and sub_tub:get() == 1 and flow_ui.Visual.Indicators.Enabled:get())
    flow_ui.Visual.Indicators.Enabled:set_visible(tab:get() == 3 and sub_tub:get() == 1)
    -- end

    -- LegBreaker
    for _, v in pairs(flow_ui.Misc.LegBreaker) do
        v:set_visible(tab:get() == 4 and sub_tub:get() == 1 and flow_ui.Misc.LegBreaker.Enabled:get())
    end
    flow_ui.Misc.LegBreaker.Enabled:set_visible(tab:get() == 4 and sub_tub:get() == 1)
    -- end 

    -- BLC 
    for _, v in pairs(flow_ui.AA.BLC) do 
        v:set_visible(tab:get() == 2 and sub_tub:get() == 1 and flow_ui.AA.BLC.Enabled:get())
    end
    flow_ui.AA.BLC.Enabled:set_visible(tab:get() == 2 and sub_tub:get() == 1)
    -- end

    -- Clantag 
    flow_ui.Misc.Clantag.Enabled:set_visible(tab:get() == 4 and sub_tub:get() == 2)
    -- end

    -- Presets
    for _, v in pairs(flow_ui.AA.Presets) do
        v:set_visible(tab:get() == 2 and sub_tub:get() == 2)
    end
    -- end

    -- Custom Jitter
    for _, v in pairs(flow_ui.AA.CustomJitter) do
        v:set_visible(tab:get() == 2 and sub_tub:get() == 3 and flow_ui.AA.CustomJitter.Type:get() ~= 0)
    end
    flow_ui.AA.CustomJitter.Type:set_visible(tab:get() == 2 and sub_tub:get() == 3)
    -- end
    ...
    flow_ui.Visual.Indicators.position_y:set_visible(false)
    -- @End
end
--- #endregion
...
local function indicators()
    ...
    local avi = flow_ui.Visual.Indicators
    ...
    if player and player:is_alive() then
        ...
        local h = 0 + render.get_text_size(font.Verdanab, "Melancholia").y
        ...
        if avi.anim_type:get() ~= 2 then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "Melancholia").x / 2 + ic.ispc, 
        ...
        if avi.show_state:get() then
            render.text(
                ...
                engine.get_screen_size().y/2 + avi.position_y:get() + render.get_text_size(font.Verdanab, "Melancholia").y,
                flow_ui.Visual.Indicators.state_color:get(), 
                ic.CurrentState, 
                true, 
                true
            )
        ...
        if ui.get_keybind_state(keybinds.fakeduck) then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 + render.get_text_size(font.Verdanab, "fd").x / 2 + render.get_text_size(font.Verdanab, "DoubleTap").x / 2 + ic.ispc,
        ...
        if ui.get_keybind_state(keybinds.safe_points) then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "sp").x - render.get_text_size(font.Verdanab, "DoubleTap").x + ic.ispc,
                engine.get_screen_size().y/2 + avi.position_y:get() + h,
                color(255,255,255,255), 
                "sp", 
                true, 
        ...
        if ui.get_keybind_state(keybinds.double_tap) or globalvars.get_dt_recharging() then
            ...
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "DoubleTap").x / 2 + ic.ispc, 
                engine.get_screen_size().y/2 + avi.position_y:get() + h,
                col, 
                "DoubleTap", 
                true, 
        ...
        if ui.get_keybind_state(keybinds.hide_shots) and not globalvars.get_dt_recharging() then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "HideShot").x / 2 + ic.ispc, 
                engine.get_screen_size().y/2 + avi.position_y:get() + h,
                color(255,255,255,255), 
                "HideShot", 
                true, 
        ...
        if ui.get_keybind_state(keybinds.hide_shots) or ui.get_keybind_state(keybinds.double_tap) or ui.get_keybind_state(keybinds.safe_points) or ui.get_keybind_state(keybinds.fakeduck) or globalvars.get_dt_recharging()     then
            h = h + render.get_text_size(font.Verdanab, "DoubleTap").y
        end
        ...
        if ui.get_keybind_state(keybinds.damage_override) then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "min dmg").x / 2 + ic.ispc, 
                engine.get_screen_size().y/2 + avi.position_y:get() + h,
                color(255,255,255,255), 
                "min dmg", 
                true, 
            ...
            h = h + render.get_text_size(font.Verdanab, " min dmg").y
        end
        -- end
        ...
        -- Drag Function
        local CurAtInd = InBounds((engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "Melancholia").x / 2 + ic.ispc),engine.get_screen_size().y/2 + avi.position_y:get(),render.get_text_size(font.Verdanab, "Flowtech").x,h)
        ...
        if (CurAtInd and globalvars.is_open_menu()) or ic.InBoundsAnim > 2 or ic.Clicked then
            ...
            render.rect_filled(
                (engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "Melancholia").x / 2 + ic.ispc), 
                engine.get_screen_size().y/2 + avi.position_y:get(), 
                render.get_text_size(font.Verdanab, "Melancholia").x, 
                h, 
...
local function leg_breaker()
    ...
    if not flow_ui.Misc.LegBreaker.Enabled:get() then return end 

    local delay = flow_ui.Misc.LegBreaker.changespeed:get()
    local tick = globalvars.get_tickcount() %(12 - delay)

    if tick == 0 then
        ui.find_menu_int("Misc.leg_movement"):set(flow_ui.Misc.LegBreaker.Type1:get())
    elseif tick == ((12 - delay) // 2) then
        ui.find_menu_int("Misc.leg_movement"):set(flow_ui.Misc.LegBreaker.Type2:get())
    end
end
--- #endregion
...
local function blc()
    ...
    if not flow_ui.AA.BLC.Enabled:get() then
        if flow_cache.blc ~= nil then
            ui.find_menu_bool("Antiaim.fake_lag"):set(flow_cache.blc)
            flow_cache.blc = nil
        end
        return 
    end 

    if flow_cache.blc == nil then flow_cache.blc = ui.find_menu_bool("Antiaim.fake_lag"):get() end
    local delay = flow_ui.AA.BLC.Speed:get()
...
local last_clantag = ""
flow_cache.clantag = false

local function clantag()

    if not flow_ui.Misc.Clantag.Enabled:get() then 
        if flow_cache.clantag then
            engine.set_clantag("")
            flow_cache.clantag = false
        end
        return
...
local function presets()
    ...
    local type = flow_ui.AA.Presets.AAPreset:get()
...
flow_cache.custom_jitter = false

local function custom_jitter()

    local type = flow_ui.AA.CustomJitter.Type:get()

    if type == 0 then 
        if flow_cache.custom_jitter then
    ...
    flow_cache.custom_jitter = true

    local delta = flow_ui.AA.CustomJitter.Delta:get()
    local left = flow_ui.AA.CustomJitter.LeftLimit:get()
    local right = flow_ui.AA.CustomJitter.RightLimit:get()
...
cheat.push_callback("on_unload", function()
    if flow_cache.clantag then engine.set_clantag("") end
    if flow_cache.blc ~= nil then ui.find_menu_bool("Antiaim.fake_lag"):set(flow_cache.blc) end
    if flow_cache.custom_jitter then ui.find_menu_int("Antiaim.yaw_offset"):set(0) end
end)
--- #endregion
...
ui.add_combobox("[visual] indicator", {"-", "style 1"})
ui.next_line()
ui.add_label("                                                        [misc]")
...
ui.add_combobox("[misc] better anim", {"-", "legs", "anim", "legs & anim"})

local font1 = render.setup_font("Verdana", 14, 500, true, false, false)  
local font = render.setup_font("Museo Sans Cyrl 500", 15, 300, true, false, true)
local font2 = render.setup_font("tahomabd", 15, 300, true, false, true)
local fonts = render.setup_font("Verdana", 12, 650, true, true, false)
local verdana = render.setup_font("Verdana", 15, 800, true, false, false )
...
indicator = function()
    ...
    self.alpha = lerpklient(self.alpha, ui.get_int("[visual] indicator") == 1 and 1 or 0, globalvars.get_frametime() * 12)
    ...
    for c_name, c_ref in pairs(self.active) do
        render.text(fontmedium, x - render.get_text_width(fontmedium, c_ref.name)/2 , y + height_offset, color.new(255, 255, 255, self.alpha*c_ref.alpha*255), c_ref.name, true)
...
if ui.get_int("[antiaim] fakelag") == 2 and cmd.get_send_packet() == true then
    ...
    if  tickcount == 1 or tickcount == 2 or tickcount == 3 or tickount == 4 or tickount == 5 or tickount == 6 or tickcount == 7 or tickcount == 8 or tickcount == 9 or tickcount == 10 or tickcount == 11 or tickcount == 12 or tickcount == 13 or tickcount == 14 or tickcount == 15 or tickcount == 16 or tickcount == 17 or tickcount == 18 or tickcount == 19 or tickcount == 20 then 
		...
		ui.set_int("Antiaim.fake_lag_limit", 1)
		ui.set_int("Antiaim.fake_lag_limit", 3)
        ui.set_int("Antiaim.fake_lag_limit", 12)
		ui.set_int("Antiaim.fake_lag_limit", 1)
        ui.set_int("Antiaim.fake_lag_limit", 2)
        ui.set_int("Antiaim.fake_lag_limit", 1)
		ui.set_int("Antiaim.fake_lag_limit", 1)
		...
		ui.set_int("Antiaim.fake_lag_limit", 10)
        ui.set_int("Antiaim.fake_lag_limit", 12)
        ui.set_int("Antiaim.fake_lag_limit", 13)
		ui.set_int("Antiaim.fake_lag_limit", 14) 
		ui.set_int("Antiaim.fake_lag_limit", 1)
		ui.set_int("Antiaim.fake_lag_limit", 1)
        ui.set_int("Antiaim.fake_lag_limit", 1)
		...
		ui.set_int("Antiaim.fake_lag_limit", 15)
    end
end
...
if ui.get_int("[antiaim] fakelag disablers") == 1 then
    ...
    if ui.get_keybind_state(keybinds.double_tap) or ui.get_keybind_state(keybinds.hide_shots) then ui.set_int("Antiaim.fake_lag_limit", 0) end
end
if ui.get_int("[antiaim] fakelag disablers") == 2 then
    ...
    if weapname == "REVOLVER" then ui.set_int("Antiaim.fake_lag_limit", 0) end
...
if ui.get_int("[antiaim] fakelag disablers") == 3 then
    ...
    if flag == 256 then ui.set_int("Antiaim.fake_lag_limit", 0) end
end

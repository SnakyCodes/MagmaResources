...
--- #region: Rawepaste UI
local tab = ui.add_combobox("Tab", {"Info", "Rage", "AntiAim", "Visual", "Misc"})
local sub_tub = ui.add_combobox("RainNailTrail [BETA] Functions", {""}) 

-- Info
acid_ui.Info.AboutLua.Name = ui.add_label("Script Name: RainNailTrail [BETA]")
acid_ui.Info.AboutLua.Version = ui.add_label("Script Version: 0~0~1 [BETA]")
acid_ui.Info.AboutLua.Creator = ui.add_label("Creator: Deyn1337#7768")
...
acid_ui.Info.AboutUser.IQ = ui.add_label("IQ: " .. math.random(0,1337))
acid_ui.Info.AboutUser.Discord = ui.add_label("Discord Server Link: https://discord.gg/9FH2WrAjhy")
-- End
...
-- AA 
-- Break Lag Comp
acid_ui.AA.BLC.Enabled = ui.add_checkbox("Enable BL")
acid_ui.AA.BLC.Speed = ui.add_sliderint("BL Update Speed", 1,10)
acid_ui.AA.BLC.InfoLabel = ui.add_label("Break Lag Compensation Work only without exploits!")
-- End

-- Presets
acid_ui.AA.Presets.AAPreset = ui.add_combobox("AA Preset", {"None", "Adaptive"})
-- End 
...
local function indicators()
    ...
    if player and player:is_alive() then
        ...
        local h = 0 + render.get_text_size(font.Verdanab, "RNT Beta").y
        ...
        if avi.anim_type:get() ~= 2 then
            render.text(
                font.Verdanab, 
                engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "RainNailTrail Beta").x / 2 + ic.ispc, 
        ...
        if avi.show_state:get() then
            render.text(
                ...
                engine.get_screen_size().y/2 + avi.position_y:get() + render.get_text_size(font.Verdanab, "RainNailTrail Beta").y,
        ...
        -- Drag Function
        local CurAtInd = InBounds((engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "RainNailTrail Beta").x / 2 + ic.ispc),engine.get_screen_size().y/2 + avi.position_y:get(),render.get_text_size(font.Verdanab, "RainNailTrail Beta").x,h)
        ...
        if (CurAtInd and globalvars.is_open_menu()) or ic.InBoundsAnim > 2 or ic.Clicked then
            ...
            render.rect_filled(
                (engine.get_screen_size().x/2 - render.get_text_size(font.Verdanab, "RainNailTrail Beta").x / 2 + ic.ispc), 
                engine.get_screen_size().y/2 + avi.position_y:get(), 
                render.get_text_size(font.Verdanab, "RainNailTrail Beta").x, 
                h, 
